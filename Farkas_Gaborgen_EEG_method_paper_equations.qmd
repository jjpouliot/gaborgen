...
Oz_fft_df %>% 
  ...
  ggplot() + 
  geom_line(aes(x=cue, y = avg_ar, group = cue, position = ggbeeswarm::geom_quasirandom())) +
...
Oz_fft_df %>% 
  ...
  ggplot(aes(x = cue, y = avg_ar)) + 
  geom_line(aes(group = participant), 
    position = ggbeeswarm::position_quasirandom(width = 0.4)) +
  geom_quasirandom(aes(color = cue),
    position = ggbeeswarm::position_quasirandom(width = 0.4)) +
  facet_wrap(
    ~block_named, nrow = 1) +
  scale_color_manual(values = cue_color) +
...
Oz_fft_df %>% 
  group_by(stan_par_id, block, cue) %>% 
  reframe(avg_ar = mean(ar)) %>% 
  mutate("block_named" = case_when(
    block == 1 ~ "Habituation",
    block == 2 ~ "Acquisition #1",
  ...
  ggplot() + 
  geom_label(aes(x = cue, y = avg_ar, color = cue, label = stan_par_id),
  # check_overlap = TRUE,
  #  position = position_jitter()
   position = position_quasirandom(groupOnX = T,width = .5)
   ) +
  facet_wrap(
    ~block_named, ncol = 1) +
  scale_color_manual(values = cue_color) +
...
Oz_fft_df %>% 
  ...
  ggplot() + 
  geom_label(aes(x = cue, y = avg_val, color = cue, label = stan_par_id),
  # check_overlap = TRUE,
  #  position = position_jitter()
   position = position_quasirandom(groupOnX = T,width = .5)
   ) +
  ...
  # # check_overlap = TRUE,
  #  position = position_jitter()
  # #  position = position_quasirandom(groupOnX = F,width = 1.5,bandwidth = .75)
  #  ) +
  facet_wrap(
    ~block_named, ncol = 1) +
  scale_color_manual(values = cue_color) +
...
Oz_fft_df %>% 
  ...
  ggplot() + 
    geom_label(aes(x = cue, y = avg_exp, color = cue, label = stan_par_id),
  # check_overlap = TRUE,
  #  position = position_jitter()
   position = position_quasirandom(groupOnX = T,width = .5)
   ) +
  # geom_text(aes(x = cue, y = avg_exp, color = cue, label = stan_par_id),
  # # check_overlap = TRUE,
