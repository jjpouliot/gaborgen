...
parameters {
  ...
  real learning_paired_average_raw; //8
  real learning_unpaired_average_raw; //9
  real learning_paired_sd_raw; //10
  real learning_unpaired_sd_raw; //11
  real scaling_mean; //12
  real scaling_sd_raw; //13
  
  array[n_participants] real<lower = 0> sigma; //37 if n_participants is 24
  array[n_participants] real intercept; //61
  array[n_participants] real fatigue; //85
  array[n_participants] real learning_paired_raw; //109
  array[n_participants] real learning_unpaired_raw; //133
...
transformed parameters {
  ...
  real learning_paired_average = inv_logit(learning_paired_average_raw);
  real learning_unpaired_average = inv_logit(learning_unpaired_average_raw);
  ...
  array[n_participants] real learning_paired = inv_logit(learning_paired_raw);
  array[n_participants] real learning_unpaired = inv_logit(learning_unpaired_raw);
...
model {
  ...
  learning_paired_average_raw ~ normal(0, 1.75); // this creates a roughly uniform(0,1), if confused visualize in R with boot::inv.logit
  learning_paired_sd_raw ~ normal(-0.5,1.5);
  learning_paired_raw ~ student_t(n_participants-1, 
                                  learning_paired_average_raw, 
                                  learning_paired_sd); // its okay the sd is not the raw version 

  learning_unpaired_average_raw ~ normal(0, 1.75);
  learning_unpaired_sd_raw ~ normal(-0.5,1.5);
  learning_unpaired_raw ~ student_t(n_participants-1, 
                                    learning_unpaired_average_raw, 
                                    learning_unpaired_sd); // its okay the sd is not the raw version 
