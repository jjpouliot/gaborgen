...
parameters {
  ...
  array[n_cues, n_phases-1] real scaling;
  // real<lower=0> cue_sd;
  // array[n_phases, n_cues] real bcue;
  real<lower=0.5,upper=1> learning_rate_average;
  real<lower=0> learning_rate_sd;
  array[n_participants] real<lower=0.5,upper=1> learning_rate;
...
model {
  ...
  scaling_sd ~ normal(0,2);
  for (q in 1:n_cues) {
    for (ph in 1:n_phases-1) {
      scaling[q, ph] ~ student_t(n_cues, 0, scaling_sd); // Multilevel prior for each participant
      
    }
  }

  sigma_average ~ normal(0,1);
  sigma_sd ~ normal(0,.25);
  ...
  real mu = intercept[participant[i]] +
            cue_habituation_baseline[cue[i]] +
...
generated quantities {
  ...
  for (i in 1:n_observations) {
    ...
    mu_pred[i] = intercept[participant[current_index]]  +
                 cue_habituation_baseline[cue[current_index]] +
                 (fatigue[participant[current_index]] * trial[current_index]) +
                 (scaling[cue[current_index]] * CSP_associative_strength[current_index]);
