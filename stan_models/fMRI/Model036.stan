functions {
  ...
  vector participant_ll(vector phi, vector theta,
    ...
    array[n_beta-15] int betas_indices_to_use = 
      data_shard_int[(6 + n_bold):(5 + n_bold + n_beta - 15)];
    ...
    // censor out paired CS+ trials from design matrix and betas
    matrix[n_bold, n_beta - 15] fit_design_matrix = design_matrix[,betas_indices_to_use];
    vector[n_beta - 15] fit_betas = betas[betas_indices_to_use];

    // Build full-length mean
    vector[n_bold] Mu_full = fit_design_matrix * fit_betas;
...
data {
  ...
  array[n_par, 44] int paired_mat_one_true;
  array[n_par, 44 - 15] int csp_linespace_per_par;
}
...
transformed data {
  ...
  int n_unpaired_csp_trials = n_trials_per_cue - n_shock_per_run;
...
//   array[n_unpaired_csp_trials] int unpaired_csp_linespace = linspaced_int_array(n_unpaired_csp_trials,1,n_unpaired_csp_trials);
  ...
  array[n_par, 44 - n_shock_per_run] int unpaired_csp_indices_per_par;
  array[n_par, n_beta - n_shock_per_run] int betas_indices_to_use_per_par;

  for(p in 1:n_par) {
    unpaired_csp_indices_per_par[p,] = csp_beta_indices[csp_linespace_per_par[p,]];
    betas_indices_to_use_per_par[p,] = 
      append_array(
        append_array(
          append_array(
            append_array(
              append_array(
                unpaired_csp_indices_per_par[p,],
                gs1_beta_indices),
              gs2_beta_indices), 
            gs3_beta_indices),
          shock_beta_indices),
        {192, 193, 194, 195, 196, 197}); //movement
  }
  ...
  // plus the beta indices (5 + n_bold + n_beta - 15)
    array[n_shards, (5 + n_bold + n_beta - 15)] int data_shard_int;
