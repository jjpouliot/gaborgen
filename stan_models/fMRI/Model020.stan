...
parameters {
  ...
  array[n_roi] vector[n_beta_stim] mu_betas_stim;
  array[n_par, n_roi] vector[n_beta - n_beta_stim] mu_betas_movement;
  ...
  array[n_roi, n_beta_stim] real <lower =0> tau_betas_stim;
  array[n_par, n_roi, n_beta - n_beta_stim] real <lower =0> tau_betas_movement;
...
transformed parameters {
  ...
  for (p in 1:n_par) {
    for (r in 1:n_roi) {
      ...
      for (b in 1:n_beta_stim){
        betas[p,r][b] = mu_betas_stim[r][b] + tau_betas_stim[r,b] * betas_z[p,r][b];
      }
      for (b in (1+n_beta_stim):n_beta) {
        betas[p,r][b] = mu_betas_movement[p,r][b-n_beta_stim] + tau_betas_movement[p,r][b-n_beta_stim]; // no multilevel prior for movement
      }
    }
  }
...
model {
  ...
  for (r in 1:n_roi) {
    ...
    mu_betas_stim[r] ~ std_normal();
    tau_betas_stim[r] ~ std_normal();
    for (p in 1:n_par) {
      mu_betas_movement[p,r] ~ std_normal();
      tau_betas_movement[p,r] ~ std_normal();
    }
  }
