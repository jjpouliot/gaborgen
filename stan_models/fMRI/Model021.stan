functions {
  ...
  //  vector[n_bold - n_censor] bold_z = bold_z_padded[1:(n_bold-n_censor)];
   ...
   matrix[n_bold, n_bold] delta_mat = 
    ...
    vector[n_bold - n_censor] bold_z = mdivide_left_tri_low(L_Cov_censor, bold_censored - Mu);
    
    real lp  = normal_lpdf(bold_z | 0, 1)   // prior on latent
               - sum(log(diagonal(L_Cov_censor)));  // Jacobian
    return [lp]';
...
transformed parameters {
  ...
  for (p in 1:n_par) {
    for (r in 1:n_roi) {
      sigma[p, r] = fmax(mu_sigma_raw[r] * 0.5 + tau_sigma_raw[r] * sigma_z[p, r], .00001); // can't be negative, I'm sure there is a better solution but probably involves priors / exp()
  ...
  // rho_time 3
  // betas 3 + n_beta
  // bold_z_padded 3 + n_beta + n_bold if included
