...
parameters {
  ...
  array[n_roi] vector[n_beta - n_motion_beta] betas_z_raw; // labeled raw because it has to because it will get the zero sum constraint per ROI
  ...
  vector[n_roi] mu_beta_csp;
  vector[n_roi] mu_beta_gs1;
  vector[n_roi] mu_beta_gs2;
  vector[n_roi] mu_beta_gs3;
  vector[n_roi] mu_beta_shock;
...
transformed parameters {
  ...
  for (r in 1:n_roi) {
    ...
    csp_L_K[r]  = cholesky_decompose( add_diag(csp_K[r], 1e-12) );
    gs1_L_K[r]  = cholesky_decompose( add_diag(gs1_K[r], 1e-12) );
    gs2_L_K[r]  = cholesky_decompose( add_diag(gs2_K[r], 1e-12) );
    gs3_L_K[r]  = cholesky_decompose( add_diag(gs3_K[r], 1e-12) );
    shock_L_K[r]= cholesky_decompose( add_diag(shock_K[r], 1e-12) );

    real temp_mu_betas_z_raw_csp_beta = mean(betas_z_raw[r][csp_beta_indices]);
    real temp_mu_betas_z_raw_gs1_beta = mean(betas_z_raw[r][gs1_beta_indices]);
    real temp_mu_betas_z_raw_gs2_beta = mean(betas_z_raw[r][gs2_beta_indices]);
    real temp_mu_betas_z_raw_gs3_beta = mean(betas_z_raw[r][gs3_beta_indices]);
    real temp_mu_betas_z_raw_shock_beta = mean(betas_z_raw[r][shock_beta_indices]);
    
    betas_z[r][csp_beta_indices] = betas_z_raw[r][csp_beta_indices] - temp_mu_betas_z_raw_csp_beta;
    betas_z[r][gs1_beta_indices] = betas_z_raw[r][gs1_beta_indices] - temp_mu_betas_z_raw_gs1_beta;
    betas_z[r][gs2_beta_indices] = betas_z_raw[r][gs2_beta_indices] - temp_mu_betas_z_raw_gs2_beta;
    betas_z[r][gs3_beta_indices] = betas_z_raw[r][gs3_beta_indices] - temp_mu_betas_z_raw_gs3_beta;
    betas_z[r][shock_beta_indices] = betas_z_raw[r][shock_beta_indices] - temp_mu_betas_z_raw_shock_beta;

    // betas[r][csp_beta_indices] = mu_beta_csp[r] + csp_L_K[r] * betas_z[r][csp_beta_indices];
    // betas[r][gs1_beta_indices] = mu_beta_gs1[r] + gs1_L_K[r] * betas_z[r][gs1_beta_indices];
    // betas[r][gs2_beta_indices] = mu_beta_gs2[r] + gs2_L_K[r] * betas_z[r][gs2_beta_indices];
    // betas[r][gs3_beta_indices] = mu_beta_gs3[r] + gs3_L_K[r] * betas_z[r][gs3_beta_indices];
    // betas[r][shock_beta_indices] = mu_beta_shock[r] + shock_L_K[r] * betas_z[r][shock_beta_indices];

    //try without zero constraint
    betas[r][csp_beta_indices] = mu_beta_csp[r] + csp_L_K[r] * betas_z_raw[r][csp_beta_indices];
    betas[r][gs1_beta_indices] = mu_beta_gs1[r] + gs1_L_K[r] * betas_z_raw[r][gs1_beta_indices];
    betas[r][gs2_beta_indices] = mu_beta_gs2[r] + gs2_L_K[r] * betas_z_raw[r][gs2_beta_indices];
    betas[r][gs3_beta_indices] = mu_beta_gs3[r] + gs3_L_K[r] * betas_z_raw[r][gs3_beta_indices];
    betas[r][shock_beta_indices] = mu_beta_shock[r] + shock_L_K[r] * betas_z_raw[r][shock_beta_indices];
  }
...
//   shock_rho ~ lognormal(log(8), .5);
  ...
  for(r in 1:n_roi) {
    betas_z_raw[r][csp_beta_indices] ~ std_normal();
    betas_z_raw[r][gs1_beta_indices] ~ std_normal();
    betas_z_raw[r][gs2_beta_indices] ~ std_normal();
    betas_z_raw[r][gs3_beta_indices] ~ std_normal();
    betas_z_raw[r][shock_beta_indices] ~ std_normal();
    mu_beta_csp[r] ~ std_normal();
    mu_beta_gs1[r] ~ std_normal();
    mu_beta_gs2[r] ~ std_normal();
    mu_beta_gs3[r] ~ std_normal();
    mu_beta_shock[r] ~ std_normal();
  }
