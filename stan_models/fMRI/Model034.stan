...
//   array[n_beta] vector[(n_roi*(n_roi-1))%/%2] rho_z_tau_betas_raw;
  ...
  vector <lower=0, upper=1> [n_roi] csp_phi; // ratio of alpha sigma gp noise
  vector <lower=0, upper=1> [n_roi] gs1_phi;
  vector <lower=0, upper=1> [n_roi] gs2_phi;
  vector <lower=0, upper=1> [n_roi] gs3_phi;
  vector <lower=0, upper=1> [n_roi] shock_phi;
...
transformed parameters {
  ...
  for (r in 1:n_roi) {
    csp_K[r] = gp_exp_quad_cov(cue_linespace, 1.0, csp_rho[r]);
    gs1_K[r] = gp_exp_quad_cov(cue_linespace, 1.0, gs1_rho[r]);
    gs2_K[r] = gp_exp_quad_cov(cue_linespace, 1.0, gs2_rho[r]);
    gs3_K[r] = gp_exp_quad_cov(cue_linespace, 1.0, gs3_rho[r]);
    shock_K[r] = gp_exp_quad_cov(shock_linespace, 1.0, shock_rho[r]);

    csp_K[r] = (1 - csp_phi[r]) * csp_K[r];
    gs1_K[r] = (1 - gs1_phi[r]) * gs1_K[r];
    gs2_K[r] = (1 - gs2_phi[r]) * gs2_K[r];
    gs3_K[r] = (1 - gs3_phi[r]) * gs3_K[r];
    shock_K[r] = (1 - shock_phi[r]) * shock_K[r];

    csp_K[r] = add_diag(csp_K[r], csp_phi[r] + 1e-9);
    gs1_K[r] = add_diag(gs1_K[r], gs1_phi[r] + 1e-9);
    gs2_K[r] = add_diag(gs2_K[r], gs2_phi[r] + 1e-9);
    gs3_K[r] = add_diag(gs3_K[r], gs3_phi[r] + 1e-9);
    shock_K[r] = add_diag(shock_K[r], shock_phi[r] + 1e-9);
    ...
    betas[r][csp_beta_indices] = csp_sigma[r] * (csp_L_K[r] * betas_z[r][csp_beta_indices]);
    betas[r][gs1_beta_indices] = gs1_sigma[r] * (gs1_L_K[r] * betas_z[r][gs1_beta_indices]);
    betas[r][gs2_beta_indices] = gs2_sigma[r] * (gs2_L_K[r] * betas_z[r][gs2_beta_indices]);
    betas[r][gs3_beta_indices] = gs3_sigma[r] * (gs3_L_K[r] * betas_z[r][gs3_beta_indices]);
    betas[r][shock_beta_indices] = shock_sigma[r] * (shock_L_K[r] * betas_z[r][shock_beta_indices]);
  }
...
//   shock_rho ~ lognormal(log(8), .5);
  ...
  csp_phi ~ beta(1.1, 1.1);
  gs1_phi ~ beta(1.1, 1.1);
  gs2_phi ~ beta(1.1, 1.1);
  gs3_phi ~ beta(1.1, 1.1);
  shock_phi ~ beta(1.1, 1.1);
