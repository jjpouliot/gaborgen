...
parameters {
  ...
  // think about what the priors should before these
  ...
  array[n_par, n_roi] real <lower =0> sigma_z; // added lower bound
...
transformed parameters {
  ...
  for (p in 1:n_par) {
    for (r in 1:n_roi) {
      ...
      sigma[p, r] = 0.5 * sigma_z[p, r];
      delta[p, r] = inv_logit(1.75 * delta_z[p, r]);
      rho_time[p, r] = inv_logit(1.75 * rho_time_z[p, r]);
      
      for (b in 1:n_beta){
        // no multilevel
        // betas[p,r][b] = mu_betas[r][b] + tau_betas[r,b] * betas_z[p,r][b];

        betas[p,r][b] = betas_z[p,r][b];
      }
    }
  }
...
model {
  ...
  // Multilevel priors (or z-score priors for the multilevel); not true for this non-multilevel model
  for (p in 1:n_par) {
    for (r in 1:n_roi) {
