data {
  ...
  array[n] int block;
  array[n] real time;
  array[n] real <lower=0, upper=1> paired;
...
parameters {
  ...
  // real scaling_average;
  // real<lower=0> scaling_sd;
  // array[n_participants] real<lower=0> scaling;
  // real<lower=0> cue_sd;
  ...
  // real cuePhase_average;
  real<lower=0> cuePhase_sd;
  array[n_cues, n_phases, n_participants] real bcuePhase;
...
model {
  ...
  fatigue_average ~ normal(0,.5);
  fatigue_sd ~ normal(0,.5);
  ...
  //   for (q in 1:n_cues) {
  //     bcue[ph, q] ~ student_t(n_cues*n_phases, 0, cue_sd);
  //   }
  // }

  // scaling_average ~ normal(0,1.5);
  // scaling_sd ~ normal(0,1);
  // scaling ~ student_t(n_participants, 0, scaling_sd) T[0,]; // Multilevel prior for each participant

  sigma_average ~ normal(0,1.5);
  sigma_sd ~ normal(0,.5);
  sigma ~ student_t(n_participants, sigma_average, sigma_sd); // Multilevel prior for each participant

  // cue_average ~ normal(0, 1.5);
  cuePhase_sd ~ normal(0, 1);
  for (q in 1:n_cues){
    for (ph in 1:n_phases) {
      for (p in 1:n_participants) {
        bcuePhase[q, ph, p] ~ student_t(n_cues * n_phases * n_participants, 0, cuePhase_sd); 

      }
    }
  }
  // learning_rate_average ~ normal(.5, 1) T[0,1];
  // learning_rate_sd ~ normal(0, .25);
  ...
  real mu = intercept[participant[i]] +
            ...
            bcuePhase[cue[i], phase[i], participant[i]];
...
generated quantities {
  ...
  for (i in 1:n_observations) {
    ...
    mu_pred[i] = intercept[participant[current_index]] +
            ...
            bcuePhase[cue[current_index], phase[current_index], participant[current_index]];
