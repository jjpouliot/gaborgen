...
parameters {
  ...
  real<lower=0> intercept_cue_sd;
  array[n_participants] real intercept_participant_avg;
...
model {
  ...
  intercept_cue_sd ~ normal(0,.25);
  for (p in 1:n_participants) {
    intercept_participant_avg[p] ~ student_t(n_participants, intercept_average, intercept_sd);
    for (q in 1:n_cues) {
      intercept[p,q] ~ student_t(n_participants*n_cues, 
                                 intercept_participant_avg, 
                                 intercept_cue_sd);
    }
  }
