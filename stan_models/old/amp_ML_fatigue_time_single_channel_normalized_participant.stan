...
model {
  ...
  fatigue_average ~ normal(0,.2);
  fatigue_sd ~ normal(0,.1);
...
generated quantities {
  ...
  for (i in 1:n_observations) {
    ...
    mu_pred[i] = intercept[participant[current_index]] +
                 (fatigue[participant[current_index]] * time[current_index]) +
                 bcue[phase[current_index], cue[current_index]];
