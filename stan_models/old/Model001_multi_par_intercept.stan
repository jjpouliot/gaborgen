...
model {
  ...
  intercept_par ~ student_t(n_participants, intercept_average, intercept_sd); // Multilevel prior for each participant
  for (p in 1:n_participants){
    for (q in 1:n_cues){
      intercept_par_cue[p, c] ~ student_t(n_cues, intercept_par[p], intercept_par_sd)
    }
  }
  ...
  for(i in 1:n_observations) {
    real mu;
    if (phase[i] == 1){
      mu = intercept[participant[indices_observed[i]]] +
           (fatigue[participant[indices_observed[i]]] * trial[indices_observed[i]]);
    } else {
      mu = intercept[participant[indices_observed[i]]] +
           ((fatigue[participant[indices_observed[i]]] + 
             bcue[phase[indices_observed[i] - 1, cue[indices_observed[i]]]] ) * 
             trial[indices_observed[i]]);
    }
...
generated quantities {
  ...
  for (i in 1:n_observations) {
    if (phase[i] == 1){
      mu_pred[i] = intercept_par_cue[participant[indices_observed[i]], cue[indices_observed[i]]] +
                   (fatigue[participant[indices_observed[i]]] * trial[indices_observed[i]]);
    } else {
      mu_pred[i] = intercept_par_cue[participant[indices_observed[i]], cue[indices_observed[i]]] +
                   (fatigue[participant[indices_observed[i]]] * trial[indices_observed[i]]) +
