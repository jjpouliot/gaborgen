...
transformed parameters {
  ...
  // A 3D array to store mu by participant, cue, and trial.
  array[n_participants, n_cues, n_trials] real mu_pct;
  array[n] real mu;
  real current_fatigue;
  real current_cue_effect;
  for(i in 1:n) {
    if (trial[i] == 1){
      current_fatigue = fatigue[participant[i]];
      current_cue_effect = 0;
      mu_pct[participant[i], , 1] = intercept_par_cue[participant[i], cue[i]];
      
      mu[i] = mu_pct[participant[i], cue[i], cue_trial_count[i]];
    } else if (phase[i] == 1){
      current_fatigue = current_fatigue + fatigue[participant[i]];
      
      mu_pct[participant[i], cue[i], cue_trial_count[i]] = 
        intercept_par_cue[participant[i], cue[i]] +
        (fatigue[participant[i]] * trial[i]);
      
      mu[i] = mu_pct[participant[i], cue[i], cue_trial_count[i]];
    } else {
      current_fatigue = current_fatigue +fatigue[participant[i]];
      current_cue_effect = current_fatigue +fatigue[participant[i]];
      
      mu_pct[participant[i], cue[i], cue_trial_count[i]] = 
        intercept_par_cue[participant[i], cue[i]] +
        (fatigue[participant[i]] * trial[i]) + 
        (bcue[phase[i] - 1, cue[i]] * phase(phase_trial_count));
