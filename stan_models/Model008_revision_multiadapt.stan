...
parameters {
  ...
  array[n_blocks] real fatigue_average; //8
  real fatigue_sd_raw; //9
  ...
  array[n_participants, n_blocks] real fatigue; //153
...
model {
  ...
  for (p in 1:n_participants) {
    ...
    //                                                        Sigma_L);
    intercept[p] ~ normal(intercept_average, intercept_sd);
    for (b in 1:n_blocks) {
      fatigue[p,b] ~ normal(fatigue_average[b], fatigue_sd);
    }
  }
  ...
  for(i in 1:n) {
    ...
    real mu = intercept[participant[i]] +
              (fatigue[participant[i], block[i]] * centered_trial);
...
generated quantities {
  ...
  for (i in 1:n_observations) {
    ...
    mu_pred[i] = intercept[participant[indices_observed[i]]] +
                 (fatigue[participant[indices_observed[i]], block[indices_observed[i]]] * centered_trial);
