kl_normal <- function(mu_prior, sd_prior, mu_post, sd_post) {
  log(sd_prior/sd_post) +
    (sd_post^2 + (mu_post - mu_prior)^2)/(2*sd_prior^2) - 0.5
}
...
trial_posterior <- data.frame(trial = numeric(),
                              CSP = numeric(),
                              GS1 = numeric(),
                              GS2 = numeric(),
                              GS3 = numeric())

trial_posterior_par_5 <- data.frame(trial = numeric(),
                              CSP = numeric(),
                              GS1 = numeric(),
                              GS2 = numeric(),
                              GS3 = numeric())

trial_posterior_par_14 <- data.frame(trial = numeric(),
                              CSP = numeric(),
                              GS1 = numeric(),
                              GS2 = numeric(),
                              GS3 = numeric())
...
for(t in 1:176) {
  ...
  trial_posterior[t,2] <- mean(model003_df$`intercept[1]` + 
    model003_df$`fatigue[1]` * t + 
    unlist(CSP_df$`scaling[1]` *  CSP_df[,t + 4]))
  
  trial_posterior[t,3] <- mean(model003_df$`intercept[1]` + 
    model003_df$`fatigue[1]` * t + 
    unlist(CSP_df$`scaling[2]` *  CSP_df[,t + 4]))
  
  trial_posterior[t,4] <- mean(model003_df$`intercept[1]` + 
    model003_df$`fatigue[1]` * t + 
    unlist(CSP_df$`scaling[3]` *  CSP_df[,t + 4]))
  
  trial_posterior[t,5] <- mean(model003_df$`intercept[1]` + 
    model003_df$`fatigue[1]` * t + 
  ...
  trial_posterior_par_5[t,2] <- mean(model003_df$`intercept[5]` + 
    model003_df$`fatigue[5]` * t + 
    unlist(CSP_df$`scaling[1]` *  CSP_df[,t + 4 + 176*4]))
  
  trial_posterior_par_5[t,3] <- mean(model003_df$`intercept[5]` + 
    model003_df$`fatigue[5]` * t + 
    unlist(CSP_df$`scaling[2]` *  CSP_df[,t + 4 + 176*4]))
  
  trial_posterior_par_5[t,4] <- mean(model003_df$`intercept[5]` + 
    model003_df$`fatigue[5]` * t + 
    unlist(CSP_df$`scaling[3]` *  CSP_df[,t + 4 + 176*4]))
  
  trial_posterior_par_5[t,5] <- mean(model003_df$`intercept[5]` + 
    model003_df$`fatigue[5]` * t + 
  ...
  trial_posterior_par_14[t,2] <- mean(model003_df$`intercept[14]` + 
    model003_df$`fatigue[14]` * t + 
    unlist(CSP_df$`scaling[1]` *  CSP_df[,t + 4 + 176*13]))
  
  trial_posterior_par_14[t,3] <- mean(model003_df$`intercept[14]` + 
    model003_df$`fatigue[14]` * t + 
    unlist(CSP_df$`scaling[2]` *  CSP_df[,t + 4 + 176*13]))
  
  trial_posterior_par_14[t,4] <- mean(model003_df$`intercept[14]` + 
    model003_df$`fatigue[14]` * t + 
    unlist(CSP_df$`scaling[3]` *  CSP_df[,t + 4 + 176*13]))
  
  trial_posterior_par_14[t,5] <- mean(model003_df$`intercept[14]` + 
    model003_df$`fatigue[14]` * t + 
...
(trial_posterior %>% 
  ggplot() +
  geom_point(data = Oz_fft_df[Oz_fft_df$participant == "113",],
...
trial_posterior_par_5 %>% 
  ggplot() +
  geom_point(data = Oz_fft_df[Oz_fft_df$participant == "118",],
             aes(x = trial, y = zamp, color = cue, shape = paired)) +
  geom_line(aes(x = trial, y = CSP), color = cue_color[1]) +
  geom_line(aes(x = trial, y = GS1), color = cue_color[2]) +
  geom_line(aes(x = trial, y = GS2), color = cue_color[3]) +
...
trial_posterior_par_14 %>% 
  ...
  geom_point(data = Oz_fft_df[Oz_fft_df$participant == "129",],
             aes(x = trial, y = zamp, color = cue, shape = paired)) +
  geom_line(aes(x = trial, y = CSP), color = cue_color[1]) +
  geom_line(aes(x = trial, y = GS1), color = cue_color[2]) +
  geom_line(aes(x = trial, y = GS2), color = cue_color[3]) +
