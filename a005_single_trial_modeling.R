...
channel_information <- readMat(paste0(
  git_repository,
  "/chanlocs_ssv4att_MRI.mat"
))

single_trial_mat_folder <- paste0(
  parent_folder,
  "/single_trial_timeseries_FFTs_CSD"
)
...
all_mat_filepaths <- all_mat_filepaths[
  !grepl(
    x = all_mat_filepaths,
    pattern = paste0(
      "participant",
...
raw_fft_paths <- all_mat_filepaths[str_detect(
  all_mat_filepaths,
  pattern = "rawChanFrequency"
)]

sliding_window_fft_paths <- all_mat_filepaths[str_detect(
  all_mat_filepaths,
  pattern = "slidingWindow_ChanFrequency"
)]

sliding_window_fft_paths <- sliding_window_fft_paths[str_detect(
  sliding_window_fft_paths,
  pattern = "600Hz.mat"
)]
...
for (path_index in 1:length(raw_fft_paths)) {
  ...
  raw_amplitude_surrounding_frequencies <- (mat_data$rawAmp[,
    ssVEP_frequency_index - 1
  ] +
    mat_data$rawAmp[, ssVEP_frequency_index + 1]) /
    2

  participant <- stringr::str_extract(
    raw_fft_paths[path_index],
    "(?<=participant)[0-9]+"
  )

  trial <- stringr::str_extract(
    raw_fft_paths[path_index],
    "(?<=trial)[0-9]+"
  ) %>%
    as.numeric()

  cue_marker <- stringr::str_extract(
    raw_fft_paths[path_index],
    "(?<=condition)S[0-9]+"
  )

  if (cue_marker == "S121") {
    paired <- rep("shock", length(amplitude)) %>%
      factor(levels = c("shock", "no_shock"))
  } else {
    paired <- rep("no_shock", length(amplitude)) %>%
      factor(levels = c("shock", "no_shock"))
  }
  ...
  cue <- case_when(
    cue_marker %in% c("S11", "S21", "S31", "S121") ~ "CSP",
  ...
  current_raw_fft_df <- data.frame(
    "participant" = rep(participant, length(amplitude)),
    "channel" = factor(
      channel_labels_in_order,
...
sliding_window_fft_df <- data.frame(
  "participant" = character(),
  "channel" = factor(levels = channel_labels_in_order),
  "trial" = numeric(),
  "phase" = factor(levels = c("habituation", "acquisition", "extinction")),
...
for (path_index in 1:length(sliding_window_fft_paths)) {
  ...
  current_sliding_window_fft_df <- data.frame(
    "participant" = rep(participant, length(amplitude)),
    "channel" = factor(
      channel_labels_in_order,
      levels = channel_labels_in_order
  ...
  sliding_window_fft_df <- rbind.data.frame(
    sliding_window_fft_df,
    current_sliding_window_fft_df
  )
}
...
fft_df <- merge(
  x = fft_df,
  y = ratings_df,
  by.x = c("participant", "block", "cue"),
  by.y = c("partInd", "ratInd", "condition"),
  all.x = T
)
...
learned_relationship_at_end_of_study <- c(
  "101",
  "102",
  "103",
  "107",
...
Oz_fft_df <- Oz_fft_df %>%
  mutate(
    learned_at_end = if_else(
      participant %in% learned_relationship_at_end_of_study,
      T,
      F
    )
  )
...
gaborgen_stan_list$n_participants <- gaborgen_stan_list$participant %>%
  unique() %>%
  length()
...
participant_by_n <- Oz_fft_df$participant %>%
  unique() %>%
  rep(each = gaborgen_stan_list$n_trials)
...
for (i in 1:(gaborgen_stan_list$n)) {
  if (!exists("current_participant")) {
    ...
    current_participant_log_filepaths <- list.files(
      path = paste0(parent_folder, "/raw_data/"),
      pattern = paste0(current_participant, "_logfile.dat$"),
      recursive = T,
      full.names = T
    )

    current_day1_log_filepath <- current_participant_log_filepaths[grepl(
      pattern = "Day1",
    ...
    if (length(current_day1_log_filepath) != 1) {
      stop(paste0(
        "more or less than one day1 log file for participant ",
        current_participant
      ))
    }

    current_day1_log_file <- read.table(
      current_day1_log_filepath,
      header = T,
      sep = ","
    )
    ...
    if (nrow(current_day1_log_file) != gaborgen_stan_list$n_trials) {
      stop(paste0(
        "Participant ",
        current_participant,
        " does not have all trials in log file. Add this person as a useable participant at top of script."
      ))
    }
  ...
  if (length(observed_trials) < current_observation) {
    ...
    time <- c(
      time,
      current_day1_log_file$timeSinceFirstTR[current_trial_number]
    )
...
model_path <- '/home/andrewfarkas/Repositories/gaborgen/stan_models/Model001_revision.stan'

#force_recompile = T is sometimes helpful
model001 <- cmdstanr::cmdstan_model(model_path, force_recompile = T)
...
model001_rev_fit <- model001$sample(
  data = gaborgen_stan_list,
  refresh = 500,
  seed = 2,
  # seed = 2,
...
model001_rev_fit_meta_data <- model001_rev_fit$metadata()
...
# ]
model001_rev_fit_relevant_parameters <- model001_rev_fit_meta_data$model_params[
  !str_detect(
    model001_rev_fit_meta_data$model_params,
    "log_lik|mu|amplitude|raw|S"
  )
]
...
model001_rev_fit_summary <-
  model001_rev_fit$summary(variables = model001_rev_fit_relevant_parameters)

model001_rev_fit_loo <- model001_rev_fit$loo()

# loo::loo_compare(model001_fit_loo)
loo::loo_compare(model001_fit_loo, model001_rev_fit_loo)
...
model_path <- '/home/andrewfarkas/Repositories/gaborgen/stan_models/Model002_revision.stan'

#force_recompile = T is sometimes helpful
model002 <- cmdstanr::cmdstan_model(model_path, force_recompile = T)
...
model002_rev_fit <- model002$sample(
  data = gaborgen_stan_list,
  refresh = 100,
  seed = 2,
  iter_warmup = warmup_samples_per_chain,
...
# Check summary to see if everything was estimated as expected
model002_rev_fit_meta_data <- model002_rev_fit$metadata()

model002_rev_fit_relevant_parameters <- model002_rev_fit_meta_data$model_params[
  !str_detect(
    model002_rev_fit_meta_data$model_params,
    "log_lik|mu_pred|amplitude|CSP_|S|raw"
  )
]

model002_rev_fit_summary <-
  model002_rev_fit$summary(variables = model002_rev_fit_relevant_parameters)

model002_rev_fit_loo <- model002_rev_fit$loo()


loo::loo_compare(model002_rev_fit_loo, model002_fit_loo)
...
loo::loo_model_weights(list(model001_fit_loo, model002_fit_loo))
...
model002_df %>%
  ...
  mutate(name = factor(name, levels = unique(name))) %>%
  ggplot() +
  geom_density_ridges(aes(x = value, y = name), stat = "binline", bins = 400) +
...
model_path <- '/home/andrewfarkas/Repositories/gaborgen/stan_models/Model003_revision.stan'

#force_recompile = T is sometimes helpful
model003 <- cmdstanr::cmdstan_model(model_path, force_recompile = T)
...
model003_rev_fit <- model003$sample(
  data = gaborgen_stan_list,
  refresh = 100,
  seed = 2,
  iter_warmup = warmup_samples_per_chain,
...
model003_rev_fit_meta_data <- model003_rev_fit$metadata()

model003_rev_fit_relevant_parameters <- model003_rev_fit_meta_data$model_params[
  # !str_detect(model003_fit_meta_data$model_params, "log_lik|mu_pred|amplitude|CSP_|S|raw")]
  !str_detect(
    model003_rev_fit_meta_data$model_params,
    "log_lik|mu_pred|amplitude|CSP_|S"
  )
]

model003_rev_fit_summary <-
  model003_rev_fit$summary(variables = model003_rev_fit_relevant_parameters)


model003_rev_fit_loo <- model003_rev_fit$loo()


loo::loo_compare(model003_rev_fit_loo, model003_fit_loo)
loo::loo_compare(
...
loo::loo_compare(model002_fit_loo, model003_fit_loo)

loo::loo_model_weights(list(model002_fit_loo, model003_fit_loo))

model003_fit_relevant_parameters <- model003_fit_meta_data$model_params[
  !str_detect(
    model003_fit_meta_data$model_params,
    "log_lik|mu_pred|amplitude|CSP_|S|raw"
  )
]
...
model003_df %>%
  ...
  mutate(name = factor(name, levels = unique(name))) %>%
  ggplot() +
  geom_density_ridges(aes(x = value, y = name), stat = "binline", bins = 100) +
  coord_cartesian(xlim = c(0, 1)) |
...
model_path <- '/home/andrewfarkas/Repositories/gaborgen/stan_models/Model004_revision.stan'

#force_recompile = T is sometimes helpful
model004 <- cmdstanr::cmdstan_model(model_path, force_recompile = T)
...
model004_rev_fit <- model004$sample(
...
model004_rev_fit_meta_data <- model004_rev_fit$metadata()
# model004_fit_meta_data <- model004_fit$metadata()

model004_rev_fit_relevant_parameters <- model004_rev_fit_meta_data$model_params[
  !str_detect(
    model004_rev_fit_meta_data$model_params,
    "log_lik|mu_pred|amplitude|raw|CSP_|S"
  )
]

model004_rev_fit_summary <-
  model004_rev_fit$summary(variables = model004_rev_fit_relevant_parameters)


model004_rev_fit_loo <- model004_rev_fit$loo()
...
model004_df %>%
  ...
  mutate(name = factor(name, levels = unique(name))) %>%
  ggplot() +
  geom_density_ridges(aes(x = value, y = name), stat = "binline", bins = 50) +
  coord_cartesian(xlim = c(0, 1)) |
...
# model_path <- '/home/andrewfarkas/Repositories/gaborgen/stan_models/Model005.stan'
model_path <- '/home/andrewfarkas/Repositories/gaborgen/stan_models/Model005_revision.stan'
#arousal centered version

#force_recompile = T is sometimes helpful
model005 <- cmdstanr::cmdstan_model(model_path, force_recompile = T)
...
model005_rev_fit <- model005$sample(
  data = gaborgen_stan_list,
  refresh = 500,
  seed = 3,
  iter_warmup = warmup_samples_per_chain,
...
model005_rev_fit_meta_data <- model005_rev_fit$metadata()
# model005_fit_meta_data <- model005_fit$metadata()

model005_rev_fit_relevant_parameters <- model005_rev_fit_meta_data$model_params[
  !str_detect(
    model005_rev_fit_meta_data$model_params,
    "log_lik|mu|amplitude|raw|S"
  )
]
# model005_fit_relevant_parameters <- model005_fit_meta_data$model_params[
#   !str_detect(
#     model005_fit_meta_data$model_params,
#     "log_lik|mu|amplitude|raw|S"
#   )
# ]

model005_rev_fit_summary <-
  model005_rev_fit$summary(variables = model005_rev_fit_relevant_parameters)
# model005_fit_summary <-
#   model005_fit$summary(variables = model005_fit_relevant_parameters)

model005_rev_fit_loo <- model005_rev_fit$loo()
# model005_fit_loo <- model005_fit$loo()

loo::loo_compare(model005_rev_fit_loo, model005_fit_loo)

## Model 006 - Arousal rating model ####

model_path <- '/home/andrewfarkas/Repositories/gaborgen/stan_models/Model006_revision.stan'
# model_path <- '/home/andrewf/Repositories/gaborgen/stan_models/Model006.stan'
#arousal centered version

#force_recompile = T is sometimes helpful
model006 <- cmdstanr::cmdstan_model(model_path, force_recompile = T)
...
model006_rev_fit <- model006$sample(
  data = gaborgen_stan_list,
  refresh = 50,
  seed = 3,
  iter_warmup = warmup_samples_per_chain,
...
model006_rev_fit_meta_data <- model006_rev_fit$metadata()

model006_rev_fit_relevant_parameters <- model006_rev_fit_meta_data$model_params[
  !str_detect(
    model006_rev_fit_meta_data$model_params,
    "log_lik|mu|amplitude|raw|S"
  )
]

model006_rev_fit_summary <-
  model006_rev_fit$summary(variables = model006_rev_fit_relevant_parameters)

model006_rev_fit_loo <- model006_rev_fit$loo()

## Model 007 - Arousal and expectancy rating model ####
model_path <- '/home/andrewfarkas/Repositories/gaborgen/stan_models/Model007_revision.stan'

#force_recompile = T is sometimes helpful
model007 <- cmdstanr::cmdstan_model(model_path, force_recompile = T)
...
model007_rev_fit <- model007$sample(
...
model007_rev_fit_meta_data <- model007_rev_fit$metadata()

model007_rev_fit_relevant_parameters <- model007_rev_fit_meta_data$model_params[
  !str_detect(model007_rev_fit_meta_data$model_params, "log_lik|mu|amplitude|S")
]
# !str_detect(model007_fit_meta_data$model_params, "log_lik|mu|amplitude|raw|S")]

model007_rev_fit_summary <-
  model007_rev_fit$summary(variables = model007_rev_fit_relevant_parameters)

model007_rev_fit_loo <- model007_rev_fit$loo()
...
loo::loo_model_weights(list(model001_fit_loo, model007_fit_loo))

model007_df <- model007_fit$draws(
  variables = model007_fit_relevant_parameters,
  format = "df"
)
...
(model007_df$b_arousal_average > 0) %>%
  sum() /
  (number_of_chains * posterior_samples_per_chain)
...
#force_recompile = T is sometimes helpful
model008 <- cmdstanr::cmdstan_model(model_path, force_recompile = T)
...
model008_fit_meta_data <- model008_rev_fit$metadata()
...
loo::loo_compare(model001_fit_loo, model003_fit_loo, model008_fit_loo)

## Model 008 revision - just adaptation slope ####
# model_path <- '/home/andrewf/Repositories/gaborgen/stan_models/Model008_revision.stan'
model_path <- '/home/andrewfarkas/Repositories/gaborgen/stan_models/Model008_revision.stan'

#force_recompile = T is sometimes helpful
model008_rev <- cmdstanr::cmdstan_model(model_path, force_recompile = T)
...
model008_rev_fit <- model008_rev$sample(
  data = gaborgen_stan_list,
  refresh = 500,
  seed = 3,
  # iter_warmup = 1000,
...
model008_rev_fit_meta_data <- model008_rev_fit$metadata()

model008_rev_fit_relevant_parameters <- model008_rev_fit_meta_data$model_params[
  !str_detect(model008_rev_fit_meta_data$model_params, "log_lik|mu|amplitude|S")
]
# !str_detect(model007_fit_meta_data$model_params, "log_lik|mu|amplitude|raw|S")]

model008_rev_fit_summary <-
  model008_rev_fit$summary(variables = model008_rev_fit_relevant_parameters)

model008_rev_fit_loo <- model008_rev_fit$loo()

loo::loo_compare(model008_fit_loo, model008_rev_fit_loo)

loo::loo_compare(model001_fit_loo, model003_fit_loo, model008_fit_loo)

## Model 008 revision - just adaptation slope multislope ####
# model_path <- '/home/andrewf/Repositories/gaborgen/stan_models/Model008_revision.stan'
model_path <- '/home/andrewfarkas/Repositories/gaborgen/stan_models/Model008_revision_multiadapt.stan'

#force_recompile = T is sometimes helpful
model008_rev_m <- cmdstanr::cmdstan_model(model_path, force_recompile = T)

#Model source code
# model008_rev_m$print()
...
model008_rev_m_fit <- model008_rev_m$sample(
  data = gaborgen_stan_list,
  refresh = 500,
  seed = 3,
  # iter_warmup = 1000,
...
model008_rev_m_fit_meta_data <- model008_rev_m_fit$metadata()

model008_rev_m_fit_relevant_parameters <- model008_rev_m_fit_meta_data$model_params[
  !str_detect(
    model008_rev_m_fit_meta_data$model_params,
    "log_lik|mu|amplitude|S"
  )
]
# !str_detect(model007_fit_meta_data$model_params, "log_lik|mu|amplitude|raw|S")]

model008_rev_m_fit_summary <-
  model008_rev_m_fit$summary(variables = model008_rev_m_fit_relevant_parameters)

model008_rev_m_fit_loo <- model008_rev_m_fit$loo()

loo::loo_compare(model008_fit_loo, model008_rev_fit_loo)
loo::loo_compare(model008_fit_loo, model008_rev_fit_loo, model008_rev_m_fit_loo)
...
save(
  ...
  file = paste0(
    parent_folder,
    "/gaborgen_eeg_manuscript_data_models_revision.RData"
  )
)
...
parent_folder <- "/home/andrewfarkas/tmp/restore3/home/andrewf/Research_data/EEG/Gaborgen24_EEG_fMRI"
...
model001_fit_meta_data <- model001_rev_fit$metadata()
...
model001_df <- posterior::as_draws_df(
  model001_rev_fit$draws(variables = model001_fit_relevant_parameters)
)

model003_fit_meta_data <- model003_rev_fit$metadata()
...
model003_df <- posterior::as_draws_df(
  model003_rev_fit$draws(variables = model003_fit_relevant_parameters)
)

# model008_fit_meta_data <- model008_fit$metadata()
# testing centered intercept
model008_fit_meta_data <- model008_rev_fit$metadata()
...
model008_df <- model008_rev_fit$draws(
  variables = model008_fit_relevant_parameters,
  format = "df"
)
...
sample_demographics <- master_excel %>%
  ...
  mutate(
    race = case_when(
      participant %in% c(139:142, 144) ~ "White",
  ...
  mutate(
    ethnicity = case_when(
      participant %in% c(139, 140, 142, 143, 144, 145) ~ "Non-Hispanic",
  ...
  mutate(
    age_years = case_when(
      participant %in% c(139, 140, 143, 144) ~ 18,
...
sample_demographics %>%
  reframe(avg_age = mean(age_years), sd_age = sd(age_years))
...
sample_demographics %>%
  ...
  reframe(
    avg_shock = mean(shock_level_mA, na.rm = T),
...
Oz_fft_df %>%
  ...
  reframe(avg_trial_count = mean(trial_count), se_trial_count = sd(trial_count))
...
Oz_fft_df %>%
  ...
  group_by(cue, block) %>%
  reframe(mean_zamp = mean(zamp), se_zamp = plotrix::std.error(zamp)) %>%
  ggplot() +
  ...
  geom_text(
    data = data.frame(
      cue = c(2.5),
      mean_zamp = c(.5, .5 - .125, .5 - .125 - .125, .5 - .125 - .125 - .125),
      block = c(4, 4, 4, 4),
      anno_label = c("CS+", "GS1", "GS2", "GS3")
    ),
    # aes(x = 2.5, y = .5,
    #   block = "4", label = "CS+")
    aes(x = cue, y = mean_zamp, label = anno_label),
    color = cue_color,
    family = "Arial",
...
# ggsave(
#   filename = "/home/andrewf/Research_data/EEG/Gaborgen24_EEG_fMRI/misc/Figure1.png",
#   dpi = 300,
#   units = "in",
#   height = 1,
#   width = 2,
...
## Loo stats ####
model001_fit_loo <- model001_rev_fit$loo()
model003_fit_loo <- model003_rev_fit$loo()
model008_fit_loo <- model008_rev_fit$loo()
...
loo::loo_compare(
  model008_fit_loo, #model 1 in paper
...
loo::loo_compare(model008_fit_loo, model001_fit_loo)

loo::loo_model_weights(list(model008_fit_loo, model001_fit_loo))
...
loo::loo_compare(
  ...
  model007_fit_loo
) # block arousal, expectancy, and arousal-expectancy slope
...
Oz_fft_df %>%
  mutate(
    participant = participant %>% as.factor() %>% as.integer() %>% as.factor
  ) %>%
  mutate(
    model001_elpd = model001_fit_loo$pointwise[, "elpd_loo"],
    model003_elpd = model003_fit_loo$pointwise[, "elpd_loo"]
  ) %>%
  mutate(elpd_1_minus_3 = model001_elpd - model003_elpd) %>%
  group_by(participant) %>%
  summarise(
    mean_elpd_1_minus_3 = mean(elpd_1_minus_3),
...
CV_by_cue_by_block_plot <-
  ...
  ggplot() +
  geom_vline(aes(xintercept = cue), color = "lightgray", linetype = "dotted") +
  geom_hline(aes(yintercept = 0), linewidth = axis_line_thickness) +
  geom_pointrange(
    aes(
      x = cue,
  ...
  geom_segment(
    data = data.frame(
      block = factor("1", levels = c("1", "2", "3", "4")),
      x = 0.3,
      xend = 0.3,
      y = 0,
      yend = positive_y
  ...
  geom_text(
    ...
    # aes(x = 2.5, y = .5,
    #   block = "4", label = "CS+")
    aes(x = cue, y = elpd_value, label = anno_label),
    color = "black",
    family = "Arial",
    size = 7.5
  ) +
...
CV_by_cue_by_block_plot +
  CV_by_participant_plot +
  plot_annotation(
    title = "Comparing Cross-Validation Contributions",
    theme = theme(
...
ggsave(
  filename = paste0(parent_folder, "/misc/Figure2_revision.png"),
  dpi = 300,
  units = "in",
  height = 1.5,
  width = 3.75,
...
adaptation_model1_avg_plot <- model008_df[samples_to_plot, ] %>%
  mutate(
    intercept_plot = intercept_average - fatigue_average * (176 + 1) / 2
  ) %>%
  ggplot() +
  ...
  geom_hline(yintercept = .5, color = "black", linetype = "dotted") +
  geom_hline(yintercept = 1, color = "black", linetype = "dotted") +
  geom_vline(xintercept = 32, color = "black", linetype = "dashed") +
  geom_vline(xintercept = 32 + 48, color = "black", linetype = "dashed") +
  geom_vline(xintercept = 32 + 48 + 48, color = "black", linetype = "dashed") +
  geom_abline(
    aes(intercept = intercept_plot, slope = fatigue_average),
    linewidth = line_width_adapt,
    alpha = line_alpha_adapt
  ) +
  scale_y_continuous(
...
median_model001_adaptation_df <- model001_df %>%
  ...
  arrange(index) %>%
  mutate(
    index = factor(index, levels = 1:gaborgen_stan_list$n_participants)
  ) %>%
  group_by(index) %>%
  mutate(
    intercept_plot = intercept - fatigue * (176 + 1) / 2
  ) %>%
  ungroup()
...
by_cue_adapt_par_median_plot <- median_model001_adaptation_df %>%
  ...
  geom_vline(xintercept = 32 + 48 + 48, color = "black", linetype = "dashed") +
  geom_abline(
    aes(intercept = intercept_plot, slope = fatigue, color = index),
...
model1_title <- ggplot() +
  ...
  theme_void() +
  theme(
    plot.title = element_text(family = "Arial", size = title_size, hjust = 0.5)
  )
...
ggsave(
  filename = paste0(parent_folder, "/misc/FigureS1_revision.png"),
  # filename = "/home/andrewf/Research_data/EEG/Gaborgen24_EEG_fMRI/misc/FigureS1.png",
  dpi = 300,
  units = "in",
  height = 3,
  width = 3,
...
learning_paired_plot <-
  ...
  scale_y_discrete(
    labels = paste0("Participant ", gaborgen_stan_list$n_participants:1)
  ) +
  scale_x_continuous(breaks = seq(0, 1, by = .5), labels = c("0", ".5", "1")) +
  coord_cartesian(
    #xlim = c(-0.01,1.01),
    expand = 0,
    ylim = c(1, 25)
...
CSP_df <- posterior::as_draws_df(
  model003_rev_fit$draws(variables = CSP_parameters)
)
...
CSP_plot_df <- CSP_df %>%
  ...
  mutate(name = factor(name, levels = unique(name))) %>%
  mutate(
    participant = rep(gaborgen_stan_list$participant, nrow(CSP_df)),
    trial = rep(gaborgen_stan_list$trial, nrow(CSP_df)),
    cue = rep(gaborgen_stan_list$cue, nrow(CSP_df)),
    paired = rep(gaborgen_stan_list$paired, nrow(CSP_df)),
    paired_unpaired_or_not_csp = factor(case_when(
      cue == 1 & paired == 1 ~ 1,
      cue == 1 & paired == 0 ~ 2,
      cue != 1 ~ 3
    ))
...
CSP_plot <- CSP_plot_df %>%
  filter(
    #participant %in% c(1,2,3),
    trial >= 30
  ) %>%
  mutate(paired = factor(paired)) %>%
  ggplot() +
  geom_vline(xintercept = 32 + 48 + 48, color = "black", linetype = "dashed") +
  geom_line(
    aes(x = trial, y = value, group = .draw),
  ...
  #     x = trial,
  #     y = 0,
  #     color = paired_unpaired_or_not_csp,
  #     group = 1
  #   ),
  #   linewidth = line_width_csp_average - .5,
  #   # alpha = line_alpha_csp,
  #   # color = "red"
  # ) +
  # geom_line(
  #   aes(x = trial, y = median_value),
  #   linewidth = line_width_csp_average,
  #   # alpha = line_alpha_csp,
  #   color = "red"
  # ) +
  scale_x_continuous(breaks = seq(40, 180, by = 20), name = "Trial") +
  scale_color_manual(values = c("red1", "lightblue", "black")) +
...
loo_R2_cmdstanr <- function(
  ...
  if (any(dim(predicted_means) != dim(log_lik_matrix))) {
    stop(
      "`predicted_means` and `log_lik_matrix` must have the same dimensions."
    )
  }
  ...
  if (!requireNamespace("extraDistr", quietly = TRUE)) {
    stop(
      "Package 'extraDistr' must be installed for Dirichlet draws (or use another method)."
    )
  }
  ...
  vary <- (rowSums(sweep(rd, 2, observed_data^2, FUN = "*")) -
...
model001_log_lik_array <- model001_rev_fit$draws(
  "log_lik",
  format = "draws_matrix"
)
model003_log_lik_array <- model003_rev_fit$draws(
  "log_lik",
  format = "draws_matrix"
)
model008_log_lik_array <- model008_rev_fit$draws(
  "log_lik",
  format = "draws_matrix"
)
...
model001_predicted_means <- model001_rev_fit$draws(
  "mu_pred",
  format = "draws_matrix"
)
model003_predicted_means <- model003_rev_fit$draws(
  "mu_pred",
  format = "draws_matrix"
)
model008_predicted_means <- model008_rev_fit$draws(
  "mu_pred",
  format = "draws_matrix"
)
...
avg_loo_r2_per_model <- data.frame(
  adaptation_loo_r2 = model008_loo_r2,
  cue_by_block_loo_r2 = model001_loo_r2,
  learning_loo_r2 = model003_loo_r2
) %>%
  mutate(
    learning_minus_adapt = learning_loo_r2 - adaptation_loo_r2,
...
for (current_par in 1:gaborgen_stan_list$n_participants) {
  ...
  loo_r2_df <- rbind(loo_r2_df, current_loo_df_entry)
}
...
learning_paired_plot +
  ...
  plot_annotation(
    title = expression(
      "Model 3 Estimated Learning Rates, Associate Value, and Cross-Validation R"^2
    ),
    theme = theme(
      plot.title = element_text(
        family = "Arial",
...
ggsave(
  filename = paste0(parent_folder, "/misc/Figure3_revision.png"),
  # filename = "/home/andrewf/Research_data/EEG/Gaborgen24_EEG_fMRI/misc/Figure3_S2.png",
  dpi = 300,
  units = "in",
  height = 4.5,
  width = 6,
...
Oz_fft_df %>%
  ...
  mutate(
    amp_diff = avg_raw_ssVEP_amp - avg_raw_surrounding_freq_amp,
...
bcue_plot <- bcue_df %>%
  ...
  scale_x_continuous(
    name = "Δ Z-Scored ssVEP",
    breaks = seq(-.75, .75, by = .25)
  ) +
  coord_cartesian(xlim = c(-.55, .55), ylim = c(1, 4.9), expand = F) +
  scale_color_manual(values = cue_color) +
  scale_fill_manual(values = cue_color) +
  ggtitle("Model 2: Effect of Cue by Block") +
  theme_classic() +
  theme(
    text = element_text(size = fig_cue_scaling_text, family = "Arial"),
    axis.title.y = element_blank(),
    axis.ticks = element_blank(),
...
scaling_plot <- model003_df %>%
  ...
  coord_cartesian(ylim = c(0, 5.375), xlim = c(-.59, 1.15), expand = F) +
  # coord_cartesian(ylim = c(0, 5.15), xlim = c(-.575, 1.25), expand = F) +
  scale_color_manual(values = cue_color) +
  scale_fill_manual(values = cue_color) +
...
set.seed(0)
samples_to_plot <- sample(
  CSP_plot_df$.draw, # further subsetting the 4,000 samples left
  size = number_of_samples_to_plot,
  replace = F
)
...
CSP_cue_trial_plot <- scaling_par_trial_df %>%
  ...
  geom_line(
    data = scaling_par_trial_avg_df %>%
      filter(participant %in% participants_to_plot),
    aes(x = trial, y = mean_value, group = name),
    color = "black",
    linewidth = 1.85
  ) +
  geom_line(
    data = scaling_par_trial_avg_df %>%
      filter(participant %in% participants_to_plot),
    aes(x = trial, y = mean_value, color = name),
    linewidth = 1.2
  ) +
  ...
  #          color = "black",
  #          size = 8) +
  scale_x_continuous(breaks = seq(0, 170, by = 20), name = "Trial") +
  scale_y_continuous(
    name = "Δ Z-Scored ssVEP",
    breaks = seq(-.25, .75, by = .5)
  ) +
  scale_color_manual(values = cue_color) +
  coord_cartesian(xlim = c(0, 176), expand = F, ylim = c(-.51, 1.1)) +
  theme_classic() +
  facet_grid(
    participant ~ .,
    # scales = "free_y",
  ...
  theme(
    text = element_text(family = "Arial", size = fig_cue_scaling_text),
    strip.background = element_blank(),
    strip.text = element_blank(),
    panel.spacing = unit(0.15, "lines"),
    legend.position = "none",
...
bcue_plot +
  ...
  CSP_cue_trial_plot +
  plot_annotation(
    title = "Model 2 vs Model 3 Cue Predictions",
    theme = theme(
      plot.title = element_text(
        family = "Arial",
        size = 30,
...
ggsave(
  filename = paste0(parent_folder, "/misc/Figure4_revision.png"),
  dpi = 300,
  units = "in",
  height = 3.5,
  width = 6,
...
mod2_acq1 <- (model001_df$`bcue[2,1]` -
  model001_df$`bcue[2,2]` -
  model001_df$`bcue[2,3]` -
  model001_df$`bcue[2,4]`)

mod2_acq2 <- (model001_df$`bcue[3,1]` -
  model001_df$`bcue[3,2]` -
  model001_df$`bcue[3,3]` -
  model001_df$`bcue[3,4]`)

mod2_acq <- (mod2_acq1 + mod2_acq2) / 2

mod2_ext <- (model001_df$`bcue[4,1]` -
  model001_df$`bcue[4,2]` -
  model001_df$`bcue[4,3]` -
  model001_df$`bcue[4,4]`)

mod3_generalization <- (model003_df$`scaling[1]` -
  model003_df$`scaling[2]` -
  model003_df$`scaling[3]` -
  model003_df$`scaling[4]`)

mod2_acq1 %>% quantile(probs = c(.025, .5, .975))
mod2_acq2 %>% quantile(probs = c(.025, .5, .975))
mod2_acq %>% quantile(probs = c(.025, .5, .975))
mod2_ext %>% quantile(probs = c(.025, .5, .975))
mod3_generalization %>% quantile(probs = c(.025, .5, .975))
...
intercept_labels <- paste0(
  "intercept[",
  1:gaborgen_stan_list$n_participants,
  "]"
)

fatigue_labels <- paste0("fatigue[", 1:gaborgen_stan_list$n_participants, "]")

fatigue_dfmod1 <- model008_rev_fit$draws(
  variables = fatigue_labels,
  format = "df"
)
intercept_dfmod1 <- model008_rev_fit$draws(
  variables = intercept_labels,
  format = "df"
)

fatigue_dfmod2 <- model001_rev_fit$draws(
  variables = fatigue_labels,
  format = "df"
)
intercept_dfmod2 <- model001_rev_fit$draws(
  variables = intercept_labels,
  format = "df"
)

fatigue_dfmod3 <- model003_rev_fit$draws(
  variables = fatigue_labels,
  format = "df"
)
intercept_dfmod3 <- model003_rev_fit$draws(
  variables = intercept_labels,
  format = "df"
)


mu_pred_dfmod3 <- model003_rev_fit$draws(
  variables = mu_pred_labels,
  format = "df"
)
...
mu_pred_long_avg_infomod3 <- mu_pred_dfmod3 %>%
  ...
  mutate(
    trial_in_study = gaborgen_stan_list$cue_trial_count[
      gaborgen_stan_list$indices_observed
    ],
  ...
  mutate(
    mean_zamp_minus_adaptation = mean_zamp -
...
mu_pred_dfmod2 <- model001_rev_fit$draws(
  variables = mu_pred_labels,
  format = "df"
)
...
mu_pred_dfmod1 <- model008_rev_fit$draws(
  variables = mu_pred_labels,
  format = "df"
)
...
hold <- Oz_fft_df %>%
  mutate(
    mean_zamp_minus_adaptation = zamp -
...
### plot calls ####
raw_GM_heat <- bind_rows(hold, hold_2) %>%
  mutate(
    cue = factor(
      cue,
      levels = c(
  ...
  geom_contour_filled(
    aes(
      z = scales::squish(
        rolling_mean,
        range = c(min(color_z_breaks) + .001, max(color_z_breaks) - .001)
      )
    ),
    breaks = color_z_breaks
  ) +
  ...
  scale_fill_manual(
    values = colorRampPalette(c(
      ...
      "red",
      "darkred"
    ))(length(color_z_breaks)), # Generate colors automatically
    name = "Mean ZAmp",
    guide = "none"
  ) +
  scale_x_continuous(
    name = "Trial",
    breaks = c(8, 8 + 12, 8 + 12 + 12, 8 + 12 + 12 + 12)
  ) +
  scale_y_continuous(
    breaks = 1:7,
  ...
  coord_cartesian(ylim = c(1, 7.6), xlim = c(1, 44), expand = F) +
  ggtitle("Grand Mean Per Cue and Trial") +
  theme_classic() +
  theme(
    text = element_text(size = heat_text_size, family = "Arial"),
    axis.ticks = element_blank(),
...
mod2_GM_heat <- bind_rows(GM_mu_pred_holdmod2, GM_mu_pred_duplicatesmod2) %>%
  mutate(
    cue = factor(
      cue,
      levels = c(
  ...
  coord_cartesian(ylim = c(1, 7), xlim = c(1, 44), expand = F) +
...
mod3_GM_heat <- bind_rows(GM_mu_pred_holdmod3, GM_mu_pred_duplicatesmod3) %>%
  mutate(
    cue = factor(
      cue,
      levels = c(
...
ggsave(
  filename = paste0(parent_folder, "/misc/Figure5_revision.png"),
  dpi = 300,
  units = "in",
  height = 4,
  width = 2.5,
...
set.seed(1)
missing_trial_samples_to_plot <- sample(
  x = 1:(8 * 5e3),
  size = missing_trial_posterior_samples,
  replace = F
)
...
participant_amp_array_labels <- paste0(
  "amplitude_all[",
  participant_trials,
  "]"
)

amplitude_array_df <- model003_rev_fit$draws(
  variables = participant_amp_array_labels,
  format = "df"
)
...
amplitude_array_df_long <- amplitude_array_df %>%
  ...
  mutate(
    participant = rep(
      rep(x = participants_to_plot, each = 176),
      missing_trial_posterior_samples
    )
  ) %>%
  mutate(
    trial = rep(
      rep(x = 1:gaborgen_stan_list$n_trials, length(participants_to_plot)),
      missing_trial_posterior_samples
    )
  ) %>%
  mutate(cue = rep(x = cue_indices, missing_trial_posterior_samples)) %>%
  mutate(
    cue = case_when(
      cue == 1 ~ "CS+",
      cue == 2 ~ "GS1",
  ...
  mutate(
    missing = name %in% paste0("amplitude_all[", participant_trial_missing, "]")
  ) %>%
  mutate(single_dot_to_plot = F)

amplitude_array_df_long[
  1:(length(participants_to_plot) * gaborgen_stan_list$n_trials),
]$single_dot_to_plot <- T
...
amplitude_array_df_long %>%
  ...
  ggplot() +
  geom_line(
    data = mu_pred_long_avg_infomod3 %>%
      filter(participant %in% participants_to_plot) %>%
      mutate(
        cue = case_when(
          cue == "1" ~ "CS+",
    ...
    aes(x = trial, y = mean_zamp, group = cue),
    color = "black",
    linewidth = 1.85
  ) +
  geom_line(
    ...
    aes(x = trial, y = mean_zamp, color = cue),
    linewidth = 1.2
  ) +
  geom_line(
    data = . %>% filter(missing == F, single_dot_to_plot),
    aes(x = trial, y = value)
  ) +
  geom_point(
    data = . %>% filter(missing == F, single_dot_to_plot),
  ...
  geom_text(
    ...
    aes(x = trial, y = value, label = anno_label),
    inherit.aes = FALSE,
    color = cue_color,
    family = "Arial",
    size = 9
  ...
  scale_color_manual(values = cue_color) +
  scale_y_continuous(name = "Z-scored ssVEP", breaks = seq(-2, 2, by = 2)) +
  scale_x_continuous(
    name = "Trial",
    breaks = seq(trial_limits[1], 175, by = 1)
  ) +
  coord_cartesian(xlim = trial_limits, ylim = c(-2.5, 2.5)) +
  theme_classic() +
  facet_wrap(
    ~participant,
    ncol = 1,
    labeller = labeller(
      participant = c("1" = "Participant 1", "14" = "Participant 14")
    )
  ) +
...
ggsave(
  filename = paste0(parent_folder, "/misc/Figure6_revision.png"),
  dpi = 300,
  units = "in",
  height = 2,
  width = 3,
...
mod2_GM_heat_minus_adapt <- bind_rows(
  GM_mu_pred_holdmod2,
  ...
  geom_contour_filled(
    aes(
      z = scales::squish(
        rolling_mean,
        range = c(
          min(color_z_breaks_minus_adapt) + .001,
  ...
  scale_color_gradientn(
    ...
    limits = c(
      min(color_z_breaks_minus_adapt),
      max(color_z_breaks_minus_adapt)
    ),
    breaks = seq(-.1, .1, by = .1)
  ) +
  coord_cartesian(ylim = c(1, 7), xlim = c(1, 44), expand = F) +
...
mod3_GM_heat_minus_adapt <- bind_rows(
  GM_mu_pred_holdmod3,
...
raw_GM_heat_minus_adapt <- bind_rows(hold, hold_2) %>%
  mutate(
    cue = factor(
      cue,
      levels = c(
...
mu_pred_data_rec <- Oz_fft_df %>%
  mutate(
    mu_pred_mod001 = mu_pred_long_avg_infomod1$mean_zamp,
...
(mu_pred_data_rec %>%
  ...
  geom_point(
    aes(x = zamp, y = mu_pred_mod001, color = cue),
    size = temp_dot_size,
  ...
  (mu_pred_data_rec %>%
    ...
    geom_abline(aes(intercept = 0, slope = 1)) +
    geom_point(
      aes(x = zamp, y = mu_pred_mod002, color = cue),
      size = temp_dot_size,
      alpha = temp_dot_alpha
    ) +
    scale_color_manual(values = cue_color) +
  ...
  (mu_pred_data_rec %>%
    ...
    geom_abline(aes(intercept = 0, slope = 1)) +
    geom_point(
      aes(x = zamp, y = mu_pred_mod003, color = cue),
      size = temp_dot_size,
      alpha = temp_dot_alpha
    ) +
    scale_color_manual(values = cue_color) +
...
Oz_fft_df %>%
  mutate(
    trial_in_block = gaborgen_stan_list$cue_trial_count[
      gaborgen_stan_list$indices_observed
    ]
  ) %>%
  ...
  mutate(
    mean_zamp_minus_adaptation = mean_zamp -
      median(
        model008_df$intercept_average + (model008_df$fatigue_average * trial)
      )
  ) %>%
  group_by(cue) %>%
  mutate(
    mean_zamp_m_adapt_moving_avg = zoo::rollapply(
      mean_zamp_minus_adaptation,
...
Oz_fft_df %>%
  ...
  stat_summary_2d(
    aes(z = mean_zamp_minus_adaptation, fill = after_stat(value)),
    bins = 25
  ) +
...
bcue_plot <- bcue_df %>%
  ...
  coord_cartesian(xlim = c(-.5, .5), ylim = c(1.55, 4.25)) +
  scale_color_manual(values = cue_color) +
  scale_fill_manual(values = cue_color) +
...
scaling_plot <- model003_df %>%
  ...
  coord_cartesian(ylim = c(0, 5.1), xlim = c(-.575, 1.25), expand = F) +
  scale_color_manual(values = cue_color) +
  scale_fill_manual(values = cue_color) +
  scale_x_continuous(name = "Δ Z-Scored ssVEP") +
...
bayes_R2_residuals <- function(
  observed_data,
  predicted_means,
  correct_for_sample_by_obs = NULL
) {
  if (!is.null(correct_for_sample_by_obs)) {
    corrected_observed_data <- -1 *
      sweep(correct_for_sample_by_obs, 2, observed_data)

    bayes_residuals <- corrected_observed_data - predicted_means
  } else {
    bayes_residuals <- -1 * sweep(predicted_means, 2, observed_data)
  }
...
model001_mu_pred_mat <- model001_rev_fit$draws(
  variables = model001_mu_pred,
  format = "df"
) %>%
...
model003_mu_pred_mat <- model003_rev_fit$draws(
...
model008_mu_pred_mat <- model008_rev_fit$draws(
  variables = model008_fit_mu_pred,
  format = "df"
) %>%
  select(starts_with("mu_pred[")) %>% #necessary to remove hidden columns .draw and such
  as.matrix()
...
current_par <- "19"
current_par <- "21"
model001_mu_pred_current_par <- model001_mu_pred_mat[,
  current_par ==
    gaborgen_stan_list$participant[gaborgen_stan_list$indices_observed]
]
model003_mu_pred_current_par <- model003_mu_pred_mat[,
  current_par ==
    gaborgen_stan_list$participant[gaborgen_stan_list$indices_observed]
]
model008_mu_pred_current_par <- model008_mu_pred_mat[,
  current_par ==
    gaborgen_stan_list$participant[gaborgen_stan_list$indices_observed]
]
...
model003_R2 <- bayes_R2_residuals(
  observed_data = gaborgen_stan_list$amplitude,
...
model008_R2 <- bayes_R2_residuals(
  observed_data = gaborgen_stan_list$amplitude,
  predicted_means = model008_mu_pred_mat
)


model001_R2 <- bayes_R2_residuals(
  observed_data = gaborgen_stan_list$amplitude,
...
loo_comapre_df %>%
  ...
  geom_text(
    aes(x = mean_2, y = mean_3, label = participant),
    check_overlap = T
  ) +
  theme_classic()
...
loo_comapre_df %>%
  ...
  #               label = participant),check_overlap = T) +
  geom_text(
    aes(x = sum_2, y = sum_3, label = block, color = cue),
    check_overlap = T
  ) +
  scale_color_manual(values = cue_color) +
  theme_classic()
...
log_likelihood_std_df <- data.frame(
  x,
...
ggsave(
  filename = "/home/andrewf/Research_data/EEG/Gaborgen24_EEG_fMRI/misc/stdnormal_log_like.png",
  dpi = 300,
  units = "in",
  height = 2,
  width = 3,
...
log_likelihood_samples_std_df <- data.frame(
  samples,
...
(loo_comapre_df %>%
  ...
  (ggplot() +
    ...
    geom_histogram(
      data = log_likelihood_samples_std_df,
      aes(x = log_likelihoods),
      bins = 1000
    ) +
    coord_cartesian(xlim = c(-3, -0.75)) +
...
loo_comapre_df %>%
  ggplot() +
  geom_density(aes(x = elpd_loo_1)) +
  geom_density(aes(x = elpd_loo_2), color = "red") +
  geom_density(aes(x = elpd_loo_3), color = "blue") +
  geom_density(
    data = log_likelihood_samples_std_df,
    aes(x = log_likelihoods),
    color = "red"
...
median_model001_adaptation_df <- model001_df %>%
  ...
  mutate(index = as.numeric(index)) %>%
  arrange(index) %>%
  mutate(index = factor(index, levels = 1:gaborgen_stan_list$n_participants))
...
model_priors <- cmdstanr::cmdstan_model(model_path, force_recompile = T)

#Model source code
# model_priors$print()

model_priors_fit <- model_priors$sample(
  refresh = 1000,
  seed = 4,
  iter_warmup = prior_samples_per_chain,
  iter_sampling = prior_samples_per_chain,
  ...
  geom_abline(
    aes(
      intercept = intercept_average -
        (fatigue_average * (gaborgen_stan_list$n_trials / 2)),
      slope = fatigue_average
    ),
    linewidth = line_width_adapt,
    alpha = line_alpha_adapt
...
adaptation_model1_avg_plot <- model008_df[samples_to_plot, ] %>%
  ...
  geom_hline(yintercept = 1, color = "black", linetype = "dotted") +
  geom_abline(
    aes(intercept = intercept_average, slope = fatigue_average),
    linewidth = line_width_adapt,
    alpha = line_alpha_adapt
  ) +
  # geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
...
geom_abline(
  data = median_model001_adaptation_df,
  aes(intercept = intercept, slope = fatigue, color = index),
  linewidth = .4,
  alpha = 1
)
...
Oz_fft_df %>%
  ...
  geom_quasirandom(
    aes(x = participant, y = sum_elpd_1_minus_3, color = cue),
    width = .1,
    alpha = 1
  ) +
  scale_color_manual(values = cue_color) +
...
Oz_fft_df %>%
  ...
  geom_line(aes(x = trial, y = moving_sum, color = cue), linewidth = 1.5) +
  scale_color_manual(values = cue_color) +
  theme_classic()
...
Oz_fft_df %>%
  ...
  ggplot() +
  geom_abline(aes(intercept = 0, slope = 1)) +
  geom_point(aes(x = model001_elpd, y = model003_elpd, color = cue)) +
  scale_color_manual(values = cue_color) +
  scale_y_continuous(breaks = seq(-10, 0, by = .5)) +
...
Oz_fft_df %>%
  ...
  geom_abline(aes(intercept = 0, slope = 1)) +
  geom_point(
    aes(x = model001_elpd, y = model003_elpd, color = cue),
    alpha = .5
  ) +
  scale_color_manual(values = cue_color) +
  theme_classic() +
...
Oz_fft_df %>%
  ...
  ggplot() +
  # geom_abline(aes(intercept = 0, slope = 1))+
  geom_point(aes(x = trial, y = elpd_1_minus_3, color = cue), alpha = .5) +
  scale_color_manual(values = cue_color) +
  theme_bw()
...
Oz_fft_df %>%
  ...
  geom_hline(aes(yintercept = 0)) +
  # geom_abline(aes(intercept = 0, slope = 1))+
  geom_line(aes(x = trial, y = mean_elpd_1_minus_3, color = cue), alpha = .5) +
  scale_color_manual(values = cue_color) +
  theme_bw()
...
model005_df <- model005_fit$draws(
  variables = model005_fit_relevant_parameters,
  format = "df"
)
...
## Sigma ####
data.frame(
  model001_average = model001_df$sigma_average,
...
data.frame(
  ...
  summarise(
    diff_mod1_mod3 = sum(model001_average > model003_average) / (8 * 5000)
  )
...
sigma_par_df <- rbind(
  model001_df %>%
    ...
    mutate(
      ...
      model = factor("model001", levels = c("model001", "model003"))
    ),
  model003_df %>%
    select(starts_with("sigma[")) %>%
    pivot_longer(starts_with("sigma[")) %>%
    mutate(
      name = factor(name, levels = unique(name)),
      model = factor("model003", levels = c("model001", "model003"))
    )
)
...
sigma_par_df %>%
  ...
  coord_cartesian(xlim = c(0.8, 1.01), ylim = c(1.4, 24.75)) +
  theme_bw()
...
normalized_sigma_par_densities <- sigma_par_df %>%
  ...
  coord_cartesian(xlim = c(0.8, 1.01), expand = 0, ylim = c(1, 25)) +
  theme_bw() +
  theme(
    axis.ticks = element_blank(),
    axis.title = element_blank(),
...
learning_rate_plot <-
  ...
  coord_cartesian(xlim = c(-0.01, 1.01), expand = 0, ylim = c(1, 25)) +
...
CSP_plot_df <- CSP_df %>%
  ...
  group_by(participant, trial) %>%
  mutate(mean_value = mean(value), median_value = median(value)) %>%
  ungroup() %>%
  filter(.draw %in% samples_to_plot)
...
CSP_plot <- CSP_plot_df %>%
  ...
  scale_x_continuous(breaks = seq(30, 180, by = 10), name = "Trial") +
  coord_cartesian(ylim = c(-0.05, 1.05), expand = F) +
  theme_classic() +
  facet_grid(
    participant ~ .,
  ...
  ggtitle(
    "Associate Value Over Trials (Black = Posterior Draws, Red = Median)"
  ) +
  theme(
    strip.text.y = element_blank(),
    panel.spacing = unit(0.15, "lines"),
...
CSP_df %>%
  ...
  pivot_longer(contains("pred")) %>%
  mutate(name = factor(name, levels = unique(name))) %>%
  filter(.draw %in% samples_to_plot, participant %in% c(1, 6, 11, 14, 21)) %>%
  ggplot(aes(x = trial, y = value, color = name)) +
  geom_line(
    aes(
      # x = trial,
  ...
  stat_summary(aes(group = name), fun = median, geom = "line", linewidth = 1) +
  # linewidth = line_width_csp,
  # alpha = line_alpha_csp) +
  ...
  #           # linewidth = line_width_csp,
  #           # alpha = line_alpha_csp) +
  scale_x_continuous(breaks = seq(0, 180, by = 10), name = "Trial") +
  scale_color_manual(values = cue_color) +
  # coord_cartesian(ylim = c(-0.05,1.05),expand = F) +
  theme_classic() +
  facet_grid(
...
amplitude_array_df <- model001_rev_fit$draws(
  variables = amplitude_array_to_plot_labels,
  format = "df"
)
...
model_priors_fit_df[samples_to_plot, ] %>%
  ...
  ggplot() +
  geom_abline(
    aes(intercept = intercept, slope = fatigue),
    size = .2,
    alpha = .2
  ) +
  scale_y_continuous(limits = c(-4, 4), breaks = seq(-4, 4, by = 1)) +
...
cov_mat <- matrix(
  intercept_average_sd,
  intercept_fatigue_average_cov,
  intercept_fatigue_average_cov,
  fatigue_average_sd
)
...
fatigue_prior_df <- data.frame(
  intercept = rnorm(n = number_of_samples_fatigue_plot, 0, .75),
...
fatigue_prior_per_par <- data.frame(
  intercept = rnorm(
    n = number_of_samples_fatigue_plot,
    0,
    sample(
      raw_deviation_samples$value_exp,
...
model001_fit_summary <-
  model001_rev_fit$summary(variables = model001_fit_relevant_parameters)
