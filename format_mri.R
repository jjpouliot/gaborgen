...
for (directory_index in 1:length(participant_directories)) {
  ...
  print(paste("Dicom 2 nifti MRI conversion for participant ", 
  ...
  current_participant_found_functional_volumes <- list.files(
    paste0(participant_directories[directory_index], "/fMRI"), 
    pattern = "EPI-cmrr-2s_.*.nii", full.names = TRUE
  )
  ...
  current_participant_found_blip_forward <- list.files(
    paste0(participant_directories[directory_index],'/fMRI'),
    pattern = 'fMRI_cmmr-DistMap_AP.*.nii', full.names = T)
  ...
  current_participant_found_blip_reverse <- list.files(
    paste0(participant_directories[directory_index],'/fMRI'),
    pattern = 'fMRI_cmmr-DistMap_PA.*.nii', full.names = T)
  ...
  if (length(list.files(paste0(participant_directories[directory_index], "/fMRI"), pattern = '^ANAT_DEOB')) > 0) {
  ...
  if (length(list.files(paste0(participant_directories[directory_index], "/fMRI"), pattern = '^FUNC_DEOB')) > 0) {
  ...
  if (length(list.files(paste0(participant_directories[directory_index], "/fMRI"), pattern = '^BLIP_FOR_DEOB')) > 0) {
  ...
  if (length(list.files(paste0(participant_directories[directory_index], "/fMRI"), pattern = '^BLIP_REV_DEOB')) > 0) {
    print('Deobliqued blip forward file already appears to exist, skipping conversion')
  } else {
    system2('tcsh', 
            args = c('-c', 
